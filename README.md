...
## 포트폴리오 기능 (v2 추가)
- 티커·수량·평단가 입력하면 실시간 주가로 평가액/수익률/비중 자동 계산 (1분 갱신)
- 챗봇이 매 대화마다 포트폴리오를 알고 답함 → 맞춤 조언 가능
- "AI 리스크 분석" 버튼 → 쏠림/손절 기준/개선 방향을 챗봇이 분석
- ⚠️ 기존 배포에 추가하는 경우: Supabase SQL Editor에서 supabase-setup.sql의
  portfolio 테이블 생성 부분(맨 아래)을 실행해야 함
